...
num_of_days=[31,30,31]
Climate_scenario=[[0.2,0.5,0.3],[0.3,0.5,0.2],[0.2,0.6,0.2]]
Climate_ratio=[]
date=[]
temperature=[]
precipitation=[]
...
output=open("/Users/yhhuang/Desktop/ClimateChange/forclimatechange/ClimateData/FutureClimateData.csv","w")
...
for i in range(3): #3 months(March~May)
    x=random.random()
...
for i in range(1,5,1):
    for j in range(3):
        ecdf=ECDF(DataArray_month[i][j+2])
        for k in range(num_of_days[j]):
            IsZero=random.random()
            if IsZero<=IszeroRate[i-1][j+2]:
...
for i in range(len(DataExport[1])):
    if int(date[i+59][6:8])<10:
        DataExport[0].append('2019/'+date[i+59][5:6]+"/"+date[i+59][7:8])
    else:
        DataExport[0].append('2019/'+date[i+59][5:6]+"/"+date[i+59][6:8])
